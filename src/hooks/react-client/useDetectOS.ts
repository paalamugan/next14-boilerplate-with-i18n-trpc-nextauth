...
import { getBitness } from '@/utils/getBitness';
...
const useDetectOS = () => {
  ...
  useEffect(() => {
    Promise.all([getBitness(), getArchitecture()]).then(([bitness, architecture]) => {
      ...
      setUserOSState({
        os: detectOS(),
        bitness: bitness === '64' || bitnessRegex.test(userAgent) ? 64 : defaultBitness,
